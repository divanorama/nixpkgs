...
let
  version = "4.9999";
  commit = "37a429ad12b4c9e6a62dbae4881a1ff03b81ab40";
  sourceRoot = "bazel-${commit}";
  patchFlags = "-p1";

  src = fetchurl {
    url = "https://github.com/bazelbuild/bazel/archive/${commit}.zip";
    sha256 = "0v0xf4s9gqm7hkmz54a7whx88vpbjd5w23dd0yqairn47a3cian7";
  };
  ...
  srcDepsSet =
    let
      ...
      (if stdenv.hostPlatform.isDarwin
       then srcs."java_tools_javac11_darwin-v10.4.zip"
       else srcs."java_tools_javac11_linux-v10.4.zip")
  ...
  remote_java_tools = stdenv.mkDerivation {
    ...
    src = srcDepsSet."java_tools_javac11_${system}-v10.4.zip";
...
stdenv.mkDerivation rec {
  ...
  buildPhase = ''
    ...
    cp ${protobuf-java}/protobuf-{java,java-util}-[0-9]*.jar ./bazel_src/derived/jars/
    export PROTOC="${protobuf}/bin/protoc"
    ${buildPackages.stdenv.cc}/bin/g++ -I "${protobuf}/include" -L "${protobuf}/lib" -lpthread -lm -lprotoc -lprotobuf bazel_src/third_party/grpc/compiler/src/java_plugin/cpp/{java_generator.h,java_generator.cpp,java_plugin.cpp} -o grpc-java-plugin
    export GRPC_JAVA_PLUGIN="$(pwd)"/grpc-java-plugin

    # Release bazel uses "version-date (short_rev)" format.
    # What we build from git isn't exactly a release version.
    # So let's produce label similar but not in identical format.
    # Note that our .bazelversion is 3.9999 as determined by
    # bazel executable name.
